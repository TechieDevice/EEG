...
raw = mne.io.read_raw_bdf('G_GS Belokrilov 1.raw.bdf')
raw.crop().load_data()
params = dict(order=4, ftype='butter')
# raw._data = mne.filter.filter_data(data = raw._data, sfreq = 125, l_freq = 2, h_freq = None, method = "iir", iir_params = params)
# raw._data = mne.filter.filter_data(data = raw._data, sfreq = 125, l_freq = None, h_freq = 40, method = "iir", iir_params = params)
raw._data = mne.filter.notch_filter(x = raw._data, Fs = 125, freqs = 60, method = "iir", iir_params = params)
write_mne_edf(raw, 'dataraw.edf', overwrite = True)